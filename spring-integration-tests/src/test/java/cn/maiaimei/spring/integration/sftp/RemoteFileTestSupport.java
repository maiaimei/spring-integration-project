...
public abstract class RemoteFileTestSupport {
   ...
   * </pre>
   * <p>
   * The intent is tests retrieve from remoteSource and verify arrival in localTarget or send from
   * localSource and verify arrival in remoteTarget.
   * <p>
   * Subclasses can change 'remote' in these names by overriding {@link #prefix()} or override this
   * method completely to create a different structure.
   * <p>
  ...
  public void setupFolders(TestInfo info) throws IOException {
    ...
    File subSourceDirectory = new File(this.sourceRemoteDirectory,
        "sub" + camelCasePrefix + "Source");
  ...
  public static File getScratchTempFolder() {
    if (scratchTemporaryFolder == null) {
      scratchTemporaryFolder = new File(
          temporaryFolder.toFile().getAbsolutePath() + File.separator + "scratch");
  ...
  protected static File getRemoteTempFolder() {
    if (remoteTemporaryFolder == null) {
      remoteTemporaryFolder = new File(
          temporaryFolder.toFile().getAbsolutePath() + File.separator + "source");
