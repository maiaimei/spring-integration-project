...
public class MoveFileFlowTest {
  ...
  public static void teardownAll() throws IOException {
    IOUtils.deleteDirectory(IOUtils.getOrCreateDirectory("/tmp/move-file"));
  }

  @Test
  public void testMoveFileFlow() throws InterruptedException {
    String name = UUID.randomUUID().toString();
    String extension = StringConstants.FILE_SUFFIX_TXT;
    IOUtils.writeStringToFile(
        IOUtils.getPath(moveFileProperties.getSource(), name + extension), name
    );

    TimeUnit.SECONDS.sleep(3);

    final List<File> files = IOUtils.listFiles(moveFileProperties.getDestination());
